...
class SingleOverlayAlignmentMark(pya.PCellDeclarationHelper):
    """
    ...
    def __init__(self):
        ...
        super(SingleOverlayAlignmentMark, self).__init__()
        ...
        self.param("cwidth", self.TypeDouble, "Cross width", default=185.0)
        self.param("clength", self.TypeDouble, "Cross length", default=185.0)
        self.param("cthick", self.TypeDouble, "Cross thickness", default=5.0)
        
        self.param("othick", self.TypeDouble, "Overlay thickness", default=5.0)
        self.param("olength", self.TypeDouble, "Overlay length", default=200.0)
        self.param("opitch", self.TypeDouble, "Overlay pitch", default=15.0)
        self.param("oN", self.TypeInt, "Number of overlay marks", default=3)
    ...
    def display_text_impl(self):
        ...
        return "SingleOverlayAlignmentMark(L=%s)" % (str(self.l))

    def produce_impl(self):
        
        scaling_factor = int(1/self.layout.dbu)
        
        lower_left = pya.DPoint(-self.width/2, -self.length/2)*scaling_factor
        upper_right = pya.DPoint(self.width/2, self.length/2)*scaling_factor
